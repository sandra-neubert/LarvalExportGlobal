...
plot_Harvest <- function(H, mpa, limitsH) {
  gg_Harvest <- ggplot2::ggplot() +
    ...
    #   limits = c(
    #     0,
    #     as.numeric(stats::quantile(dplyr::pull(Hort, val), 0.95))
    #   ),
    #   oob = scales::squish,
    ...
    ggplot2::scale_fill_distiller(
      ...
      limits = limitsH, #c(
       # 0,
        #as.numeric(stats::quantile(dplyr::pull(H, val), 0.95))
      #),
      oob = scales::squish,
      guide = ggplot2::guide_colourbar(
        title.position = "bottom",
  ...
  gg_dist <- ggplot2::ggplot() +
    ...
    scale_fill_cmocean(name = "deep",
                       ...
                       guide = guide_colourbar(
                         ...
                         frame.colour = "black"),
                       limits = c(0,1500)) +
    #  labs(fill = expression('Distance to MPA (km)'))+
    # ggplot2::scale_fill_distiller(
    #   name = "Distance (km)",
    #   palette = "Purples", # "YlOrRd",#"YlGnBu",
    #   direction = -1,
    #   # breaks = seq(round(limitsH[1], digits = 1),round(limitsH[2]-0.08, digits=1),length.out=5),
    #   # # low = "#fff5eb",
    #   # # high = "#d94801",
    ...
    #   #limits = limitsH, #c(
    #   # 0,
  #   #as.numeric(stats::quantile(dplyr::pull(mpa_dist, val), 0.95))
  #   #),
  #   oob = scales::squish,
  #   guide = ggplot2::guide_colourbar(
  #     title.position = "bottom",
